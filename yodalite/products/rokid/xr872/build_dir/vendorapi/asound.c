...
#include "../../../../../../vendor/xr872/project/example/audio_play/audio.h"
...
#define SAMPLE_CHANNEL      4
#define SAMPLE_16K_30MS     480   /* for 16 kHz */
#define SAMPLE_RATE         16000
#define SAMPLE_LENGTH       SAMPLE_SIZE*SAMPLE_CHANNEL*SAMPLE_16K_30MS
...
static struct pcm_config xr871_pcm_read_init(struct asound_resource *pasound_res)
{
    ...
    pAudio.pcm_cfg.channels = 4;//pasound_res->channels;
    pAudio.pcm_cfg.format = pasound_res->format;
    pAudio.pcm_cfg.period_count = pasound_res->period_count;
    pAudio.pcm_cfg.period_size = pasound_res->period_size;
    pAudio.pcm_cfg.rate = pasound_res->sample_rate;
    pAudio.pcm_cfg.mix_mode = 1;
    data_buff = malloc(SAMPLE_LENGTH);
...
static int play_mp3_iis(void)
{
    ...
    ret = player->play(player, "flash://0?addr=1228864&length=17626");  //1200k
    if (ret != 0) {
...
{  
   ...
   if (fd == RAW_ASR)
   {
    pcm_asound_count = snd_pcm_read(&pcm_raw_asr_config, AUDIO_CARD_ID, data_buff,SAMPLE_LENGTH);
    for(int i =0;i<SAMPLE_16K_30MS ;i++)
    {
