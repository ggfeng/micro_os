...
//#define COAPSERIP       "coap://10.88.9.15"
...
#define BOOT_STRAP      "/device/inform/bootstrap"
#define HEART_BEAT      "/device/inform/heartbeat"
#define BOOT            "/device/inform/boot"
#define NOTIFY_DATA            "/device/inform/data"
#define UNBIND            "/device/command/unbind"
#define COAPBROADIP     "coap://10.89.255.255"
#define REGIST          "/qlink/regist"
...
//static const char* gatewayIP = "10.88.9.15";
...
void InitClientData()
{
    ...
    wait_seconds = 2;	/* default timeout in seconds */
...
coap_new_request(coap_context_t *ctx, method_t m, coap_list_t *options ) {
  ...
  if ( !coap_add_token(pdu, the_token.length, the_token.s)) {
    debug("cannot add token to request\n");
      LOGI("cannot add token to reques");
  }
...
resolve_address(const str *server, struct sockaddr *dst) {
  ...
  //LOGI("resolve_address--addrstr is %s ",addrstr);
  //error = getaddrinfo(addrstr, "", &hints, &res);
  error = getaddrinfo(addrstr, PORTSTR, &hints, &res);
  //LOGI("resolve_address--%s ; result is %d",gai_strerror(error),error);
  if (error != 0) {
  ...
  if (received->hdr->code == COAP_RESPONSE_CODE(205)) {
    ...
    if (sent && coap_check_option(received, COAP_OPTION_SUBSCRIPTION, &opt_iter)) {
      debug("observation relationship established, set timeout to %d\n", OBS_SECONFS);
        LOGI("observation relationship established, set timeout to %d",OBS_SECONFS);
    ...
    if (!block_opt) {
      ...
      if (coap_get_data(received, &len, &databuf)) {
        printf("%s:%d:>>>>>>>%s,len:%d\n", __FUNCTION__, __LINE__, databuf, len);
          LOGI("message_handler>>>>>>>%s,len:%d", databuf, len);
        *responsebuf = (char*)malloc(len);
        if (*responsebuf) {
...
int Qlink_SendCaopRequest(const char* url, const char *opt, const char *data, char ** response, int *len) {
  InitClientData();
  coap_context_t  *ctx = NULL;
  ...
  char port_str[NI_MAXSERV] = PORTSTR;
  int res;
  ...
  method = cmdline_method((char*)opt);
  ...
  if (!ctx) {
    LOGI("Qlink_SendCaopRequest----->cannot create context----> ");
  ...
  if (! (pdu = coap_new_request(ctx, method, optlist))){
      LOGI("Qlink_SendCaopRequest----->coap_new_request not success----> ");
      return -1;
  }
  ...
  debug("timeout is set to %d seconds\n", wait_seconds);
    //LOGI("timeout is set to %d seconds\n", wait_seconds);
...
static inline int ParseResponse(char *url, char *data)
{
    char *reponse = NULL;
    int replen;
    if (Qlink_SendCaopRequest(url, "post", data, &reponse, &replen) == -1) {
        if (reponse) free(reponse);
    } else {
        if (reponse) {
            LOGI("ParseResponse: %s\n",reponse);
...
static inline int ParseBootResponse(char *url, char *data)
{
    char *reponse = NULL;
    int replen;
    if (Qlink_SendCaopRequest(url, "post", data, &reponse, &replen) == -1) {
        ...
        if (reponse) {
            LOGI("ParseBootResponse: %s\n",reponse);
/*            if (strncmp(reponse, "{\"respCode\":1}", replen) == 0) {
                free(reponse);
                return 1;
            } else {
                free(reponse);
            ...
            if(strstr(reponse, "\"respCode\":1000") == NULL){
                free(reponse);
                return 1;
            }else{
                free(reponse);
...
int GetParseResponse(char *url, char *data, char *reponsedata, int *len)
{
    char *reponse = NULL;
    int replen;
    if (Qlink_SendCaopRequest(url, "post", data, &reponse, &replen) == -1) {
		...
		LOGI("GetParseResponse: %s\n",reponse);
        if (reponse) {
			if(replen > 512)
				replen = 512;
            strncpy(reponsedata, reponse, replen);
			*len = replen;
            free(reponse);
            return 1;
        }
...
int Qlink_notifyGatewayJoinBoot(const char* deviceMac, const char* deviceType)
{
  ...
  char data[500] = {0};
  snprintf(data, 500, "{\"deviceMac\":\"%s\",\"deviceType\":\"%s\"}", deviceMac, deviceType);
...
int Qlink_Reg(const char* deviceMac,const char* deviceType,const char* productToken,const char* timestamp, char *reponsedata, int *len)
{
  ...
  char url[500] = { 0 };
  snprintf(url, 500, "%s:%s%s", COAPSERIP, PORTSTR, BOOT_STRAP);
  printf("reg url:%s\n",url);

  char data[500] = {0};
  snprintf(data, 500, "{\"deviceMac\":\"%s\",\"deviceType\":\"%s\",\"productToken\":\"%s\",\"timestamp\":\"%s\"}", deviceMac, deviceType,productToken,timestamp);
  LOGI("data is %s",data);
  return GetParseResponse(url, data, reponsedata, len);
}
...
{
  char url[500] = { 0 };
  snprintf(url, 500, "%s%s", broadcast, COAPSUCESS);
  printf("Broadreg url:%s\n",url);
...
{
  char url[500] = { 0 };
  snprintf(url, 500, "%s%s", broadcast, REGIST);
  printf("Broadreg url:%s\n",url);

  char data[500] = {0};
  snprintf(data, 500, "{\"respCode\":\"1\",\"respCont\":\"Success\",\"deviceId\":\"%s\",\"deviceType\":\"%s\"}", deviceId, deviceType);
  return ParseResponse(url, data);
}
...
{
  char url[500] = { 0 };
  snprintf(url, 500, "%s:%s%s", COAPSERIP, PORTSTR, HEART_BEAT);

  printf("Heartbeat url:%s\n",url);
  char data[500] = {0};
  snprintf(data, 500, "{\"deviceId\":\"%s\"}", deviceId);
  return ParseBootResponse(url, data);
}
...
int Qlink_boot(const char* deviceId,const char* deviceType,const char* firmwareVersion,const char* softwareVersion, const char* ipAddress, const char* timestamp)
{
  char url[500] = { 0 };
  int ret = -1;
  snprintf(url, 500, "%s:%s%s", COAPSERIP, PORTSTR, BOOT);
  printf("reg url:%s\n",url);

  char data[500] = {0};
  snprintf(data, 500, "{\"deviceId\":\"%s\",\"deviceType\":\"%s\",\"firmwareVersion\":\"%s\",\"softwareVersion\":\"%s\",\"ipAddress\":\"%s\",\"timestamp\":\"%s\"}", deviceId, deviceType,firmwareVersion, softwareVersion, ipAddress, timestamp);
  LOGI("Qlink_boot:%s\n",data);
  ret = ParseBootResponse(url, data);
  if(ret == 1){
        Qlink_notify_data(deviceId,"Inform",timestamp,firmwareVersion,softwareVersion);
  }
  return ret;
...
{
  char url[500] = { 0 };
  snprintf(url, 500, "%s:%s%s", COAPSERIP, PORTSTR, NOTIFY_DATA);

  //LOGI("Qlink_notify_data url:%s\n",url);
  char data[500] = {0};
  snprintf(data, 500, "{\"deviceId\":\"%s\",\"eventType\":\"%s\",\"timestamp\":\"%s\",\"data\":{\"params\":[{\"paramCode\":\"firmware\",\"paramValue\":\"%s\"},{\"paramCode\":\"softVersion\",\"paramValue\":\"%s\"}]}}", deviceId, eventType,timestamp,firmwareVersion,softwareVersion);
  LOGI("Qlink_notify_data data:%s\n",data);
  return ParseResponse(url, data);
}
