...
static void aes_expand_key(uint8_t *key, uint8_t *expkey)
{
...
static void aes_encrypt(uint8_t *in, uint8_t *expkey, uint8_t *out)
{
...
static void aes_decrypt(uint8_t *in, uint8_t *expkey, uint8_t *out)
{
	uint8_t state[AES_STATECOLS * 4];
	int round;
...
void aes_pkcs5pading(uint8_t *inout)
{
	uint8_t len = 0;
	uint8_t pad = 0;
	uint8_t i = 0;
	...
	len = strlen(inout);
	pad = AES_KEY_LENGTH - len;
#ifdef AES_DEBUG
	printf("pad is 0x%x\n", pad);
#endif
	for (i = len; i < AES_KEY_LENGTH; i++) {
		inout[i] = pad;
	}
}
...
void aes_cbc_encrypt_blocks(uint8_t *key_exp, uint8_t *src, uint8_t *iv, uint8_t *dst, uint32_t num_aes_blocks)
{
	...
	//uint8_t *cbc_chain_data = "Rokid";
	uint32_t i;

	memcpy(cbc_chain_data, iv, strlen(iv));
	for (i = 0; i < num_aes_blocks; i++) {
#ifdef AES_DEBUG
		printf("encrypt_object: block %d of %d\n", i, num_aes_blocks);
#endif
		if (strlen(src) < AES_KEY_LENGTH) {
			aes_pkcs5pading(src);
		}
...
void aes_cbc_decrypt_blocks(uint8_t *key_exp, uint8_t *src, uint8_t *iv, uint8_t *dst, uint32_t num_aes_blocks)
{
	...
	//uint8_t cbc_chain_data[AES_KEY_LENGTH] = "Rokid";
	uint32_t i;
...
void aes_do_encrypt(uint8_t *src, uint8_t *key, uint8_t *iv, uint8_t *cipher)
{
	...
	src_p = (uint8_t*)malloc(aes_blocks * AES_KEY_LENGTH);
	memset(src_p, 0 ,aes_blocks * AES_KEY_LENGTH);
	strcpy(src_p, src);
	aes_cbc_encrypt_blocks(key_exp, src_p, iv, cipher, aes_blocks);
	free(src_p);
}
