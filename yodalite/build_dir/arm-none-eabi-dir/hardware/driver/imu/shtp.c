...
static void updateCallbacks(void)
{
    ...
    for (chanNo = 0; chanNo < SH2_MAX_CHANS; chanNo++) {
        ...
        appName = 0;
        for (appNo = 0; appNo < SH2_MAX_APPS; appNo++) {
        ...
        if (appName == 0) {
            ...
            for (listenerNo = 0; listenerNo < SH2_MAX_CHANS; listenerNo++) {
