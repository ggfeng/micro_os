...
static onUnbind onUnbindHandle = NULL;
...
void Qlink_setUnbindCallback(onUnbind Fun)
{
    if(Fun) {
        onUnbindHandle = Fun;
    }
...
//add by suchenglong for unbind start
void
hnd_command_unbind(coap_context_t  *ctx, struct coap_resource_t *resource,
	     coap_address_t *peer, coap_pdu_t *request, str *token,
	     coap_pdu_t *response) {
    size_t size;
    unsigned char *data;
    LOGI("hnd_command_unbind--->is called\n");
    /* if my_clock_base was deleted, we pretend to have no such resource */
    response->hdr->code = COAP_RESPONSE_CODE(201);
    coap_get_data(request, &size, &data);
    if (size == 0) {		/* re-init */
        LOGI("hnd_command_unbind--->client post none\n");
        return;
    } else {
        LOGI("%s:%d:>>>>>>>%s,len:%d\n", __FUNCTION__, __LINE__, data, size);
        if(onUnbindHandle != NULL){
            char *tmp = "{\"respCode\":1}";
            unsigned char buf[3];
            response->hdr->code = COAP_RESPONSE_CODE(205);
            ...
            LOGI("%s:%d:>>>>>>> goto callback\n", __FUNCTION__, __LINE__);
            onUnbindHandle(data, size);
        }
    }
}
//add by suchenglong for unbind end
void 
...
check_async(coap_context_t  *ctx, coap_tick_t now) {
  ...
  if (!response) {
    ...
    LOGI("check_async: insufficient memory, we'll try later");
    async->appdata = 
  ...
  if (coap_send(ctx, &async->peer, response) == COAP_INVALID_TID) {
    debug("check_async: cannot send response for message %d\n", 
	  response->hdr->id);
LOGI("check_async: cannot send response for message %d\n",
     	  response->hdr->id);
  }
...
//add by suchenglong for unbind start
  r = coap_resource_init((unsigned char *)"device/command/unbind", 21, 0);
  coap_register_handler(r, COAP_REQUEST_POST, hnd_command_unbind);
  coap_add_attr(r, (unsigned char *)"command", 7, (unsigned char *)"unbind", 6, 0);
  coap_add_resource(ctx, r);
//add by suchenglong for unbind end
}
